...
def cmd_extract(args) -> int:
    ...
    extractor = Extractor(pdf_path, out_dir)
    ...
    print(f"   Cobertura textual: {100 * extractor.stats.paginas_con_texto_nativo / extractor.stats.total_paginas:.1f}%")
    print(f"   Outputs en: {out_dir}/")
...
def cmd_audit(args) -> int:
    ...
    try:
        ...
        print(f"⚠️  JSON con advertencias, generando auditoría de todos modos: {e.error_count()} errores", file=sys.stderr)
    ...
    out_audit = guardar_auditoria(catalogo, out_dir, sha_pdf, fecha=args.fecha or date.today().isoformat())
